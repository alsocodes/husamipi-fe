...
export default function Forum() {
  ...
  const columns = [
    ...
    { field: 'title', label: 'Nama Forum', sort: true },
    {
      field: '_',
      label: 'Aksi',
      ...
      func: ({ row }) => (
        <div className='flex gap-1 flex-col md:flex-row'>
          ...
          <button
            disabled={!accesses?.includes('forum_update')}
          ...
          <button
            disabled={!accesses?.includes('forum_delete')}
            ...
            onClick={() => {
              if (confirm(`Anda akan menghapus forum ${row.name}?`))
                dispatch(DeleteForum(row.id));
            }}
          >
        ...
        <section className='pt-10 pb-40 px-6 md:px-0'>
          <div className='container mx-auto'>
            <MyTable
              ...
              addButton={
                <button
                  disabled={!accesses?.includes('forum_create')}
