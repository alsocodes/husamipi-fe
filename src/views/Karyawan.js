...
export default function Karyawan() {
  ...
  const { data, progress, formResult } = useAppSelector(selectUser);
  ...
  const columns = [
    ...
    { field: 'role.name', label: 'Role', sort: true },
    {
      field: 'isAdmin',
      label: 'Is Admin',
      sort: true,
      func: ({ value }) => (value === true ? 'Y' : 'T'),
    },

    {
      ...
      func: ({ row }) => (
        <div className='flex gap-1 flex-col md:flex-row'>
          ...
          <button
            disabled={!accesses?.includes('user_update')}
          ...
          <button
            disabled={!accesses?.includes('user_delete')}
            ...
            onClick={() => {
              if (confirm(`Anda akan menghapus user ${row.name}?`))
  ...
  return (
    <div className='w-full h-screen overflow-y-auto'>
      ...
      <main className='w-full'>
        ...
        <BreadCrumb title={'Data User/Karyawan'} />
        ...
        <section className='pt-10 pb-40 px-6 md:px-0'>
          <div className='container mx-auto'>
            <MyTable
              ...
              addButton={
                <button
                  disabled={!accesses?.includes('user_create')}
