    ...
    { field: 'product.name', label: 'Produk', sort: true },
    {
      ...
      func: ({ row }) => (
        <div className='flex gap-1 flex-col md:flex-row'>
          ...
          <button
            disabled={!accesses?.includes('elearning_update')}
          ...
          <button
            disabled={!accesses?.includes('elearning_delete')}
            ...
            onClick={() => {
              if (confirm(`Anda akan menghapus materi ${row.name}?`))
                dispatch(DeleteMateri(row.id));
            }}
          >
        ...
        <section className='pt-10 pb-40 px-6 md:px-0'>
          <div className='container mx-auto'>
            <MyTable
              ...
              addButton={
                <button
                  disabled={!accesses?.includes('elearning_create')}
